...
dM2_dq2 = zeros(1,length(local_V));
dmu_dq2 = zeros(1,length(local_V));
...
for j = 1:length(M_crit)
    ...
    for i = 1:length(local_V)
        ...
        Q = 1 + 0.5*(heat_capacity_ratio - 1) * (free_stream_mach_number)^2 * (1 - sq_clamped_local_velocity./(free_stream_velocity)^2);
    
        derivative_consts = 0.5 * (heat_capacity_ratio - 1) * (1./(free_stream_velocity)^2) * (free_stream_mach_number)^2;
        dM2_dq2(i) = sq__clamped_local_mach_number .* ((1./sq_clamped_local_velocity) + derivative_consts.*(1./Q));
        
        dmu_dq2(i) = dM2_dq2(i) * dmu_dM2(i);
        
        local_mach(i) = sqrt(sq__clamped_local_mach_number);

    end
    txt = ['M_{crit} = ', num2str(M_crit(j))];
    plot(local_mach, dmu_dq2, 'DisplayName', txt, 'LineWidth',1.75, 'Color', map(j,:))
    hold on
end
...
set(gca,'FontSize',14)
grid on;
ylabel('$\frac{\partial \mu_f}{\partial |q|^2} \,[s^2/m^2]$','Interpreter','latex','FontSize',20)
xlabel('$M_{local}\,[-]$','Interpreter','latex','FontSize',20)
title('$\frac{\partial \mu_f}{\partial |q|^2}$','Interpreter','latex','FontSize',22)
legend show
